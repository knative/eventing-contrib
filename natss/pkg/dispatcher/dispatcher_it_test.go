...
// This dispatcher tests the full integration of the dispatcher code with NATSS.
// This test doesn't run on the CI because unit tests script doesn't start a NATSS cluster.
...
// Start a NATSS cluster with docker: docker run --rm --net=host nats-streaming:0.17.0
...
func TestDispatcher(t *testing.T) {
	...
	dispatcher, err := NewDispatcher(dispatcherArgs)
	if err != nil {
		t.Fatal(err)
	}
	...
	channela := messagingv1alpha1.Channel{
		...
		Spec: messagingv1alpha1.ChannelSpec{
			Subscribable: &eventingduck.Subscribable{
				Subscribers: []eventingduck.SubscriberSpec{{
					...
					ReplyURI:          mustParseUrl(t, channelBProxy.URL),
					DeadLetterSinkURI: mustParseUrl(t, deadLetterServer.URL),
		...
		Status: messagingv1alpha1.ChannelStatus{
			AddressStatus: duckv1alpha1.AddressStatus{
				Address: &duckv1alpha1.Addressable{Addressable: duckv1beta1.Addressable{URL: mustParseUrl(t, "http://channela.svc")}},
			},
		},
	}
	channelb := messagingv1alpha1.Channel{
		ObjectMeta: metav1.ObjectMeta{
			Namespace: "default",
			Name:      "channelb",
		},
		...
		Status: messagingv1alpha1.ChannelStatus{
			AddressStatus: duckv1alpha1.AddressStatus{
				Address: &duckv1alpha1.Addressable{Addressable: duckv1beta1.Addressable{URL: mustParseUrl(t, "http://channelb.svc")}},
			},
		},
	}

	err = dispatcher.ProcessChannels(context.TODO(), []messagingv1alpha1.Channel{channela, channelb})
	if err != nil {
		t.Fatal(err)
	}

	failed, err := dispatcher.UpdateSubscriptions(context.TODO(), &channela, false)
	if err != nil {
		t.Fatal(err)
	}
	if len(failed) != 0 {
	...
	failed, err = dispatcher.UpdateSubscriptions(context.TODO(), &channelb, false)
	if err != nil {
		t.Fatal(err)
	}
	if len(failed) != 0 {
