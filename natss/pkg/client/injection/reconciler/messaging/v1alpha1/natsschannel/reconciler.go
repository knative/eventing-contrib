...
import (
	...
	messagingv1alpha1 "knative.dev/eventing-contrib/natss/pkg/client/listers/messaging/v1alpha1"
...
func (r *reconcilerImpl) Reconcile(ctx context.Context, key string) error {
	...
	original, err := r.Lister.NatssChannels(namespace).Get(name)
...
func (r *reconcilerImpl) updateStatus(existing *v1alpha1.NatssChannel, desired *v1alpha1.NatssChannel) error {
	existing = existing.DeepCopy()
	return reconciler.RetryUpdateConflicts(func(attempts int) (err error) {
		// The first iteration tries to use the injectionInformer's state, subsequent attempts fetch the latest state via API.
		if attempts > 0 {
			existing, err = r.Client.MessagingV1alpha1().NatssChannels(desired.Namespace).Get(desired.Name, metav1.GetOptions{})
			if err != nil {
				return err
			}
		}
		...
		_, err = r.Client.MessagingV1alpha1().NatssChannels(existing.Namespace).UpdateStatus(existing)
		return err
	})
}
...
func (r *reconcilerImpl) updateFinalizersFiltered(ctx context.Context, resource *v1alpha1.NatssChannel) (*v1alpha1.NatssChannel, error) {
	...
	actual, err := r.Lister.NatssChannels(resource.Namespace).Get(resource.Name)
	if err != nil {
		return resource, err
	}
	...
	resource, err = r.Client.MessagingV1alpha1().NatssChannels(resource.Namespace).Patch(resource.Name, types.MergePatchType, patch)
	if err != nil {
