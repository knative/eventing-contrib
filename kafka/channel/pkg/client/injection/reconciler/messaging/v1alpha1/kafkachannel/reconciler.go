...
func (r *reconcilerImpl) Reconcile(ctx context.Context, key string) error {
	...
	original, err := r.Lister.KafkaChannels(namespace).Get(name)
...
func (r *reconcilerImpl) updateStatus(existing *v1alpha1.KafkaChannel, desired *v1alpha1.KafkaChannel) error {
	...
	return reconciler.RetryUpdateConflicts(func(attempts int) (err error) {
		...
		if attempts > 0 {
			existing, err = r.Client.MessagingV1alpha1().KafkaChannels(desired.Namespace).Get(desired.Name, metav1.GetOptions{})
			if err != nil {
				return err
			}
		}
		...
		_, err = r.Client.MessagingV1alpha1().KafkaChannels(existing.Namespace).UpdateStatus(existing)
		return err
	})
}
...
func (r *reconcilerImpl) updateFinalizersFiltered(ctx context.Context, resource *v1alpha1.KafkaChannel) (*v1alpha1.KafkaChannel, error) {
	...
	actual, err := r.Lister.KafkaChannels(resource.Namespace).Get(resource.Name)
	if err != nil {
		return resource, err
	}
	...
	resource, err = r.Client.MessagingV1alpha1().KafkaChannels(resource.Namespace).Patch(resource.Name, types.MergePatchType, patch)
	if err != nil {
