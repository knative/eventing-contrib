...
import (
	...
	"knative.dev/eventing-contrib/couchdb/importer/pkg/reconciler/resources"
	...
	"knative.dev/eventing-contrib/couchdb/importer/pkg/apis/sources/v1alpha1"
	versioned "knative.dev/eventing-contrib/couchdb/importer/pkg/client/clientset/versioned"
	listers "knative.dev/eventing-contrib/couchdb/importer/pkg/client/listers/sources/v1alpha1"
)

const (
	// Name of the corev1.Events emitted from the reconciliation process
	couchdbimporterReconciled         = "CouchDbImporterReconciled"
	couchdbimporterReadinessChanged   = "CouchDbImporterReadinessChanged"
	couchdbimporterUpdateStatusFailed = "CouchDbImporterUpdateStatusFailed"
	couchdbimporterDeploymentCreated  = "CouchDbImporterDeploymentCreated"
	couchdbimporterDeploymentUpdated  = "CouchDbImporterDeploymentUpdated"
...
var (
	...
	couchDbEventTypes = []string{
		v1alpha1.CouchDbImporterChangesEventType,
	}
)

// Reconciler reconciles a CouchDbImporter object
type Reconciler struct {
	*reconciler.Base
	...
	couchdbimporterLister listers.CouchDbImporterLister
...
// converge the two. It then updates the Status block of the CouchDbImporter
...
func (r *Reconciler) Reconcile(ctx context.Context, key string) error {
	...
	// Get the CouchDbImporter resource with this namespace/name
	original, err := r.couchdbimporterLister.CouchDbImporters(namespace).Get(name)
	if apierrors.IsNotFound(err) {
		// The resource may no longer exist, in which case we stop processing.
		logging.FromContext(ctx).Error("CouchDbImporter key in work queue no longer exists", zap.Any("key", key))
		return nil
	} else if err != nil {
		return err
	}
	...
	couchdbimporter := original.DeepCopy()

	// Reconcile this copy of the CouchDbImporter and then write back any status
	// updates regardless of whether the reconcile error out.
	err = r.reconcile(ctx, couchdbimporter)
	if err != nil {
		logging.FromContext(ctx).Warn("Error reconciling CouchDbImporter", zap.Error(err))
	} else {
		logging.FromContext(ctx).Debug("CouchDbImporter reconciled")
		r.Recorder.Eventf(couchdbimporter, corev1.EventTypeNormal, couchdbimporterReconciled, `CouchDbImporter reconciled: "%s/%s"`, couchdbimporter.Namespace, couchdbimporter.Name)
	}

	if _, updateStatusErr := r.updateStatus(ctx, couchdbimporter.DeepCopy()); updateStatusErr != nil {
		logging.FromContext(ctx).Warn("Failed to update the CouchDbImporter", zap.Error(err))
		r.Recorder.Eventf(couchdbimporter, corev1.EventTypeWarning, couchdbimporterUpdateStatusFailed, "Failed to update CouchDbImporter's status: %v", err)
...
func (r *Reconciler) reconcile(ctx context.Context, source *v1alpha1.CouchDbImporter) error {
...
func (r *Reconciler) createReceiveAdapter(ctx context.Context, src *v1alpha1.CouchDbImporter, sinkURI string) (*appsv1.Deployment, error) {
	...
	if apierrors.IsNotFound(err) {
		...
		r.Recorder.Eventf(src, corev1.EventTypeNormal, couchdbimporterDeploymentCreated, "Deployment created, error: %v", err)
		return ra, err
	} else if err != nil {
		return nil, fmt.Errorf("error getting receive adapter: %v", err)
	} else if !metav1.IsControlledBy(ra, src) {
		return nil, fmt.Errorf("deployment %q is not owned by CouchDbImporter %q", ra.Name, src.Name)
	} else if r.podSpecChanged(ra.Spec.Template.Spec, expected.Spec.Template.Spec) {
		ra.Spec.Template.Spec = expected.Spec.Template.Spec
		if ra, err = r.KubeClientSet.AppsV1().Deployments(src.Namespace).Update(ra); err != nil {
			return ra, err
		}
		r.Recorder.Eventf(src, corev1.EventTypeNormal, couchdbimporterDeploymentUpdated, "Deployment updated")
		return ra, nil
	} else {
...
func (r *Reconciler) reconcileEventTypes(ctx context.Context, src *v1alpha1.CouchDbImporter) error {
...
func (r *Reconciler) getEventTypes(ctx context.Context, src *v1alpha1.CouchDbImporter) ([]eventingv1alpha1.EventType, error) {
...
func (r *Reconciler) makeEventTypes(src *v1alpha1.CouchDbImporter) ([]eventingv1alpha1.EventType, error) {
	eventTypes := make([]eventingv1alpha1.EventType, 0)

	// Only create EventTypes for Broker sinks.
	// We add this check here in case the CouchDbImporter was changed from Broker to non-Broker sink.
...
func (r *Reconciler) computeDiff(current []eventingv1alpha1.EventType, expected []eventingv1alpha1.EventType) ([]eventingv1alpha1.EventType, []eventingv1alpha1.EventType) {
	...
	// This could happen if the CouchDbImporter CO changes its broker.
...
func (r *Reconciler) getReceiveAdapter(ctx context.Context, src *v1alpha1.CouchDbImporter) (*appsv1.Deployment, error) {
...
func (r *Reconciler) getLabelSelector(src *v1alpha1.CouchDbImporter) labels.Selector {
	return labels.SelectorFromSet(resources.Labels(src.Name))
}

func (r *Reconciler) updateStatus(ctx context.Context, desired *v1alpha1.CouchDbImporter) (*v1alpha1.CouchDbImporter, error) {
	couchdbimporter, err := r.couchdbimporterLister.CouchDbImporters(desired.Namespace).Get(desired.Name)
	if err != nil {
		return nil, err
	}

	// If there's nothing to update, just return.
	if reflect.DeepEqual(couchdbimporter.Status, desired.Status) {
		return couchdbimporter, nil
	}

	becomesReady := desired.Status.IsReady() && !couchdbimporter.Status.IsReady()

	// Don't modify the informers copy.
	existing := couchdbimporter.DeepCopy()
	existing.Status = desired.Status

	cj, err := r.couchdbClientSet.SourcesV1alpha1().CouchDbImporters(desired.Namespace).UpdateStatus(existing)
	if err == nil && becomesReady {
		duration := time.Since(cj.ObjectMeta.CreationTimestamp.Time)
		r.Logger.Infof("CouchDbImporter %q became ready after %v", couchdbimporter.Name, duration)
		r.Recorder.Event(couchdbimporter, corev1.EventTypeNormal, couchdbimporterReadinessChanged, fmt.Sprintf("CouchDbImporter %q became ready", couchdbimporter.Name))
		if err := r.StatsReporter.ReportReady("CouchDbImporter", couchdbimporter.Namespace, couchdbimporter.Name, duration); err != nil {
			logging.FromContext(ctx).Sugar().Infof("failed to record ready for CouchDbImporter, %v", err)
		}
	}
...
// MakeEventSource computes the Cloud Event source attribute for the given importer
func (r *Reconciler) makeEventSource(src *v1alpha1.CouchDbImporter) (string, error) {
